...
const (
	...
	// IPAMClusterPoolMap are IP pool definitions used for the cluster-pool-v2beta2 IPAM mode.
	IPAMClusterPoolMap = "cluster-pool-map"
...
type OperatorConfig struct {
	...
	// IPAMClusterPoolMap are IP pool definitions used for the cluster-pool-v2beta2 IPAM mode.
	IPAMClusterPoolMap map[string]string
...
func (c *OperatorConfig) Populate(vp *viper.Viper) {
	...
	if m, err := command.GetStringMapStringE(vp, IPAMClusterPoolMap); err != nil {
		log.Fatalf("unable to parse %s: %s", IPAMClusterPoolMap, err)
	} else {
		c.IPAMClusterPoolMap = m
	}
}
...
var Config = &OperatorConfig{
	...
	IPAMClusterPoolMap:       make(map[string]string),
